...
var PositionRead = cardinal.NewReadType[PositionRequest, PositionReply]("position")
```

And register those objects with `world.RegisterTransactions` and `world.RegisterReads`, the following RPC endpoints will be created in Nakama:

- /tx/game/move-a-player
- /read/game/position

RPC requests to those Nakama endpoints will be passed along to your Cardinal implementation. Transactions will be added to the transaction queue (which can be processed at the next game tick in a registered System), and Reads will be passed directly to Cardinal.
...
When submitting transactions from Nakama to Cardinal, transactions must be cryptographically signed. The Cardinal plugin will use an existing private key, or generate one if one does not exist. This key generation and signing happens automatically. To make use of this automatic signing, a client must first register a >>>Persona Tag<<<(LINK_TO_MARKDOWN). Once a Persona Tag has been requested and accepted, future transactions will be automatically signed.
...
|------------------------|-------------------|--------------------------------------|--------------------------------------------------|
| private_key_collection | private_key_key   | 00000000-0000-0000-0000-000000000000 | The actual private key                           |
...
4) Sends a request to Cardinal's `/tx/persona/create` endpoint.
...
Cardinal exposes the endpoint `/transaction-receipts`, however Nakama does not generate a corresponding RPC for this endpoint. Clients shouldn't call this endpoint directly. Instead, see the >>>Streaming Transaction Receipts(LINK_TO_MARKDOWN)<<< section for instructions on how to consume these receipts.
