...
Transactions and Queries registered with a `cardinal.World` object will automatically get Nakama RPC endpoints created for them. For example, if you set up a Transaction and a Query with the following code:
...
And register those objects with `world.RegisterTransactions` and `world.RegisterQueries`, the following set of RPC endpoints will be created in Nakama:
