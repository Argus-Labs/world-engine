...
The starter-game-template uses Nakama's storage layer to store a global cryptographic key and Persona Tag information for each user. See the >>>Cardinal Plugin<<<(LINK_TO_MARKDOWN) page for more details.
