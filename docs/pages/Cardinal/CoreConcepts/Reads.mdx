...
`address:port/read/foo-bar`. (TODO: update this after we finalize how we want to handle endpoints)
...
<Callout type={"warning"}>
    Not all Go types are supported when using `WithReadEVMSupport`. <Link href="/cardinal/CoreConcepts/evm-support">Read More</Link>
</Callout>
