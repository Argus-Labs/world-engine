...
func main() {
	...
	err := ecs.RegisterComponent[LocationComponet](world)
	if err != nil {
		log.Fatal(err)
	}
	err = ecs.RegisterComponent[AttackPowerComponent](world)
	if err != nil {
		log.Fatal(err)
	}
	err = ecs.RegisterComponent[HealthComponent](world)
	if err != nil {
		log.Fatal(err)
	}
}
