...
transactions, and reads, as well as ticking the game simulation forward. `World`'s in Cardinal use [Redis](https://redis.io/) to store the game's state.
...
Using the `inmem` package, you can get an instantiated `World` object that uses an in-memory data store, [miniredis](https://github.com/alicebob/miniredis). This is the simplest way to get started with building and testing your game. This should NOT be used in production.

### With Redis Instance

To instantiate a production ready `World` object, you need to instantiate a `RedisStorage` object, and create a
...
`RegisterComponents` must only be called once. Subsequent calls to this method will return an error.
</Callout>
...
```
<Callout type={"warning"}>
`RegisterTransactions` must only be called once. Subsequent calls to this method will return an error.
</Callout>
...
```
<Callout type={"warning"}>
`RegisterReads` must only be called once. Subsequent calls to this method will return an error.
</Callout>
...
<Callout type={"warning"}>
   `LoadGameState` must be called after components and transactions have been registered. `LoadGameState` can be called at most once. Subsequent calls to this method will return an error.
</Callout>
...
The code above will incur a tick every 500 milliseconds, which will update the game 2 times every second.

## Creating Entities

Entities can be created in the world in two different ways:


`Create` accepts one or more components and returns the newly created entity's ID.

```go
// create one entity with the Health and Attack component.
id, err := world.Create(component.Health, component.Attack)
if err != nil {
    log.Fatal(err)
}
```

`CreateMany` does the same thing, but takes the amount of entities to create as the first parameter, and
returns a slice of entity IDs.


```go
// create 10 entities with the Health and Attack component.
ids, err := world.Create(10, component.Health, component.Attack)
if err != nil {
    log.Fatal(err)
}
```
