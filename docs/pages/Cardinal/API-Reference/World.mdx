...
The `WithAdapater` world option enables `World` objects to read transactions from the EVM base shard, and rebuild game state from those transactions. When running World Engine in production, the `WithAdapter` option must be used.
