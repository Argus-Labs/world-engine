...
// AttackReceipt is the receipt for the Attack transaction.
type AttackReceipt struct {
	Success bool
}

// AttackTx is an attack transaction.
var AttackTx = ecs.NewTransactionType[AttackMsg, AttackReceipt](
	"attack",
	ecs.WithTxEVMSupport[AttackMsg, AttackReceipt],
)
```
...
#### Example
```go
txID := AttackTx.AddToQueue(w, AttackMsg{TargetID: 3502})
```
...
```go
AttackTx.SetResult(w, txID, AttackReceipt{Success: true})
```
