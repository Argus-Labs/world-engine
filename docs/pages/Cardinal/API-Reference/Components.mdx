...
var HealthComponent = ecs.NewComponentType[Health](
	ecs.WithDefault(Health{Amount: 100, Capacity: 100}),
)
```
...
```go
entity, err := HealthComponent.First(w)
```
...
| Type             | Description                                      |
|------------------|--------------------------------------------------|
| `storage.EntityID` | The ID of the first entity with the component. |
| `error`          | Returns an error if no entity is found.          |


### MustFirst

`MustFirst` returns the first entity that has the component. MustFirst will panic if any errors are encountered.
...
```go
entity := HealthComponent.MustFirst(w)
```
...
|-----------|----------------------|--------------------------------------------------|
| `w`       | `*World`             | Pointer to the ECS world object.                 |
| `id`      | `storage.EntityID`   | Entity ID of the entity to remove the component from. |
...
|-----------|----------------------|--------------------------------------------------|
| `w`       | `*World`             | Pointer to the ECS world object.                 |
| `id`      | `storage.EntityID`   | Entity ID of the entity to add the component to. |
