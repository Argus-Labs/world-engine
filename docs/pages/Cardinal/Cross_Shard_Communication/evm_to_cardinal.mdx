...
The `query` and `sendMessage` Router methods both take in a paremter of `bytes`. These must be formed by ABI encoding a struct with the exact same field types as their Cardinal game shard defines them.
...
Full example of a solidity contract utilizing the router for messages, results, and queries:
