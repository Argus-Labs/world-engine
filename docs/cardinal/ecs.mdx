...
Declare an archetype as a struct of `WithComponent[C]` fields. A search field registers
those components before the world starts, so they are available during snapshot restoration.
The declarations carry no entity data. Queries return world-bound `Entity` handles.
...
`Poisoned` registers an optional component without requiring it in the `Mobs` search.
...
Creation adds exactly the archetype's components, initialized to zero. The archetype must
be registered first. A `Contains[Mob]` or `Exact[Mob]` field registers it automatically.
Without a search field, call `w.RegisterArchetype[Mob]()` before starting the world.
`state.Mobs.Create()` also remains available and returns `(EntityID, Entity)`.
...
`Iter` yields each matching entity's numeric ID and handle:
...
Use `Filter(func(EntityID, Entity) bool)` and `Limit` to narrow results.
`Single()` returns `(EntityID, Entity, error)` and requires exactly one match.
...
Register optional types before startup with a `WithComponent[T]` system field or an
archetype declaration. They do not need to be part of the query that found the entity.
