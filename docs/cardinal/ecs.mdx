...
func BuffSystem(state *BuffSystemState) error {
	now := state.Tick()
	for mob := range state.Contains[Mob]().Iter() {
		buffs := mob.Get[Buffs]()
...
A system builds queries from its `BaseSystemState` while it runs. A query selects entities
by component type:

- `state.Contains[T]()` matches entities with every component in `T`, allowing extras.
- `state.Exact[T]()` matches entities with exactly the components in `T`.

`T` is an archetype: a struct whose exported fields are component types. The struct carries
no entity data; it names a set of components. Every component in it must be registered with
`w.RegisterComponent[C]()` before the world starts. Nothing registers components implicitly.
Building a query that names an unregistered component panics. Queries return world-bound
`Entity` handles.
...
func MobSystem(state *MobSystemState) {
    for mob := range state.Contains[Mob]().Iter() {
        // ...
    }
}
...
Nothing about a query happens at `RegisterSystem`, so systems and components can be
registered in any order, including across plugins. The first query for an archetype
resolves its component set with reflection and caches it on the world. Later calls are a
map lookup with no allocation, so build the query inside the system body. Build it once per
run when the same query is iterated several times.
...
does the same for the query's archetype: `state.Exact[Mob]().Create()`.
...
```go
mob, err := state.Contains[Mob]().GetByID(entityID)
if err != nil {
