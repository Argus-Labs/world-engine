...
this way, and registering a system that references an unregistered component panics.
...
w.RegisterSystem(SetupSystem, cardinal.WithHook(cardinal.PreUpdate))
```
...
Searches select entities by component type:

- `Contains[T]` matches entities with every declared component, allowing extras.
- `Exact[T]` matches entities with exactly the declared components.

Declare an archetype as a struct of `WithComponent[C]` fields. Every component in it must
be registered with `w.RegisterComponent[C]()` before the system that uses it is registered.
Nothing registers components implicitly. Registering a system whose fields name an
unregistered component panics. The declarations carry no entity data. Queries return
world-bound `Entity` handles.

```go
type Mob struct {
    Health   cardinal.WithComponent[Health]
    Position cardinal.WithComponent[Position]
}

type MobSystemState struct {
    cardinal.BaseSystemState
    Mobs     cardinal.Contains[Mob]
    Poisoned cardinal.WithComponent[Poisoned]
}
...
`Poisoned` declares an optional component without requiring it in the `Mobs` search.

### Creating an Entity

Use the generic `Create[T]` method inherited from `BaseSystemState`:

```go
func SpawnMob(state *MobSystemState) {
    mob := state.Create[Mob]()
...
if any component in `T` was not registered with `w.RegisterComponent`. It does not need a
search field for `T`. `state.Mobs.Create()` also remains available and returns an `Entity`.
...
that ID in a system, bind it to the system's world:

```go
entity := state.Entity(entityID)
...
restoration, `state.Entity(savedID)` binds an ID to the restored world. IDs belong to one
...
```go
func DamageMobs(state *MobSystemState) {
    for mob := range state.Mobs.Iter() {
...
`GetByID` validates the search's component requirements before returning a handle:

```go
mob, err := state.Mobs.GetByID(entityID)
if err != nil {
    // The entity is absent or its components do not match the search.
    return
}
health := mob.Get[Health]()
```

Use `state.Entity(entityID)` when no archetype check is needed. Binding does not itself
...
A `WithComponent[T]` system field checks that at registration time. They do not need to be
part of the query that found the entity.
...
structural changes while iterating a search, so moving rows cannot skip other matches.
