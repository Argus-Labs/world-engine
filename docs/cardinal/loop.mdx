...
Cardinal exports OpenTelemetry traces over OTLP gRPC. Point `OTEL_EXPORTER_OTLP_ENDPOINT` at a collector (default `jaeger:4317`) and set `OTEL_TRACE_SAMPLE_RATE` between `0.0` and `1.0` to control how many ticks are sampled. An empty endpoint disables tracing.

Each tick is one root trace named `cardinal.tick` with these children:

| Span | Attributes |
|:--|:--|
| `cardinal.tick` | `cardinal.tick.height`, `cardinal.tick.commands` (commands drained this tick) |
| `cardinal.system` | `cardinal.system.name`, `cardinal.system.hook` (one span per system run) |
| `cardinal.events.dispatch` | error status when any event handler fails |
| `cardinal.event.publish` | `cardinal.event.name`, `cardinal.event.recipient`, `cardinal.event.subscribers`, `cardinal.event.waiters` (one per event) |
| `cardinal.command.send` | `cardinal.command.name`, `cardinal.command.target` (one per inter-shard command) |
| `cardinal.persist_state` | `cardinal.snapshot.due` (present only on ticks that serialize state) |

Startup emits `cardinal.init` (init systems, also on debug reset) and `cardinal.restore` (snapshot load).

**Commands.** The `SendCommand` request span (from ConnectRPC) and the NATS `handler.command.*` span (from another shard) carry `cardinal.command.name`. The request finishes before the tick that processes the command, so the tick is not its child. Instead, `cardinal.tick` holds one span link per drained command, tagged with `cardinal.command.name` and `cardinal.command.persona`. Follow the link from the tick to find the request, or search the request's trace for linked ticks.
