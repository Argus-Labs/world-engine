...
Startup emits `cardinal.init` (init systems, also on debug reset) and `cardinal.restore` (snapshot load). ConnectRPC and NATS handlers are traced separately and are not parents of tick spans, because ticks are driven by the clock rather than by a request.
