...
System is how game logic is implemented in Cardinal. Each system is executed once at every tick and is responsible for handling messages and updating the state of the game. 
...
<Tip>
If you are coming from EVM development background, you might notice that this behavior is in stark contrast to how smart contract works. 
    
In smart contracts, game state can only be updated when a transaction calls a function of the contract. In Cardinal, game state is updated via system at every tick regardless of whether there are transactions.

This makes it easy to implement state updates that need to consistently happen at every time step/interval (e.g. regeneration, gravity, etc.) which EVM smart contracts are not able to do seamlessly.
</Tip>

### All game states must be stored in components
...
By convention, systems are defined in the `system` directory in its own separate files.

You can easily create a new system and register it to the world by following these steps:

<Steps>
    <Step title="Create the system function">
    ...
    <Step title="Register the system in main.go">
