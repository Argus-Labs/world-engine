...
func AttackSystem(worldCtx cardinal.WorldContext) error {
	msg, err := cardinal.GetMessage[msg.AttackPlayerRequest, msg.AttackPlayerResponse](worldCtx)
	if err != nil {
		return err
    }

	// Iterate over all Attack messages
	msg.Each(worldCtx, func(attack cardinal.TxData[AttackInput]) (AttackOutput, error) {
