...
<Note>
  Not all go types are supported for the `Request` & `Response` struct fields when using this option. See [EVM+ Message and Query](/cardinal/game/evm) to learn more.
</Note>
