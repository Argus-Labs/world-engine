...
func createPersonaSystem(wCtx WorldContext) error {
	return EachMessage[CreatePersona](wCtx, func(tx message.TxType[CreatePersona]) (any, error) {
		...
		if err != nil {
			if eris.Is(err, sign.ErrSignatureValidationFailed) {
				// If the transaction is not signed, set the persona signer to an empty string
				// This should only happen when Cardinal is running with transaction verification disabled
				wCtx.Logger().Warn().Msg("Transaction not signed: 0x0 address will be used")
				signer = common.Address{}
			} else {
				// If the transaction is signed but the signer address cannot be recovered, return an error
				return nil, err
			}
		}
		...
		err = wCtx.personaManager().index.update(tx.Msg().PersonaTag, signer.Hex(), id)
		if err != nil {
			return nil, err
		}
...
func authorizePersonaAddressSystem(wCtx WorldContext) error {
	return EachMessage[AuthorizePersonaAddress](wCtx, func(tx message.TxType[AuthorizePersonaAddress]) (any, error) {
		// Check if the Persona Tag exists
		persona, id, err := wCtx.GetPersona(tx.PersonaTag())
		if err != nil {
			return nil, eris.Errorf("persona %s does not exist", tx.PersonaTag())
		}

		// Check that the authorizer is the owner of the persona
		signer, err := tx.Signer()
		if err != nil {
			return nil, eris.Wrap(err, "failed to get signer")
		}
		if persona.SignerAddress != signer.Hex() {
			return nil, eris.Errorf("persona %s is not owned by the authorizing address", tx.PersonaTag())
		}

		// Normalize address
		address := strings.ToLower(tx.Msg().Address)
		address = strings.ReplaceAll(address, " ", "")

		// Check that address is valid
		valid := common.IsHexAddress(address)
		if !valid {
			return nil, eris.Errorf("address %s is invalid", address)
		}
		...
		err = UpdateComponent[Persona](wCtx, id, func(s *Persona) *Persona {
			...
			s.AuthorizedAddresses = append(s.AuthorizedAddresses, address)
			return s
		})
		if err != nil {
