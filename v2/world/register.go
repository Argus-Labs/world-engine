...
func RegisterMessage[Msg message.Message](w *World, opts ...message.Option[Msg]) error {
	var msg Msg
	_, exists := w.registeredMessages[msg.Name()]
	if exists {
		return eris.Errorf("message %q is already registered", msg.Name())
	} else {
		w.registeredMessages[msg.Name()] = message.NewMessageType[Msg](opts...)
	}
	return nil
}
