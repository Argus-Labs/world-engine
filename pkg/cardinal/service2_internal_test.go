...
func TestService2_SendCommand(t *testing.T) {
	t.Parallel()

	t.Run("happy path", func(t *testing.T) {
		t.Parallel()
		prng := testutils.NewRand(t)
		fixture := newService2Fixture(t, prng, false)
	...
	t.Run("wrong address rejected", func(t *testing.T) {
		...
		_, err = fixture.svc.SendCommand(
			service2TestContext(testutils.RandString(prng, 8)),
			connect.NewRequest(&cardinalv1.SendCommandRequest{Command: cmdPb}),
		)
		require.Error(t, err)
...
func TestService2_Query(t *testing.T) {
	t.Parallel()
...
func TestService2_PublishDefaultEvent(t *testing.T) {
	t.Parallel()
...
func TestService2_PublishInterShardCommand(t *testing.T) {
	t.Parallel()
	...
	t.Run("happy path", func(t *testing.T) {
		...
		fixtureA := newService2Fixture(t, prng, true)
		fixtureB := newService2Fixture(t, prng, true)
...
func newService2Fixture(t *testing.T, prng *rand.Rand, registerNATSEndpoints bool) *service2Fixture {
	t.Helper()
	...
	svc := newService2(w, AuthModePassthrough, "")
	svc.registerCommandHandler(testutils.SimpleCommand{}.Name())
	w.service = svc

	fixture := &service2Fixture{
		svc:       svc,
		world:     w,
		commandID: cmdID,
	}
	...
	if registerNATSEndpoints {
		...
		require.NoError(t, microService.AddGroup("command").AddEndpoint(testutils.SimpleCommand{}.Name(), svc.handleInterShardCommand))
...
func service2TestContext(userID string) context.Context {
