...
type SpanRing struct {
	...
	mask uint64
	head uint64
	// lower bound for Snapshot iteration; prevents returning zeroed slots after Reset(nextHead) where nextHead > len(buf)
	validFrom       uint64
	maxSpansPerTick int
...
// Reset clears all buffered spans and sets head to nextHead so the next
// StartTick call will expect that tick height. Use after a world reset or
// restore to re-synchronize the ring with the new tick sequence.
func (r *SpanRing) Reset(nextHead uint64) {
	r.mu.Lock()
	defer r.mu.Unlock()

	for i := range r.buf {
		r.buf[i].TickHeight = 0
		r.buf[i].TickStart = time.Time{}
		r.buf[i].Spans = r.buf[i].Spans[:0]
	}
	r.head = nextHead
	r.validFrom = nextHead
	r.droppedSpans.Store(0)
}

// DroppedSpans returns the total number of spans dropped because a tick
// exceeded maxSpansPerTick. The counter is reset by Reset().
func (r *SpanRing) DroppedSpans() uint64 {
	return r.droppedSpans.Load()
}

// Snapshot returns a copy of all valid tick spans in chronological order.
// Only ticks that were actually started (via StartTick) are included;
// zeroed slots left by Reset are excluded.
