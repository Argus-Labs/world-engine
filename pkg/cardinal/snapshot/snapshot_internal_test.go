...
func TestUnmarshalSnapshotNegative(t *testing.T) {
	...
	t.Run("rejects empty component name", func(t *testing.T) {
		...
		invalidWorldState, err := proto.Marshal(&cardinalv1.WorldState{
			Columns: []*cardinalv1.Column{{Name: ""}},
		})
		require.NoError(t, err)
...
func randomSnapshot(prng *rand.Rand) *cardinalv1.Snapshot {
	...
	worldState := &cardinalv1.WorldState{
		NextId:        prng.Uint32(),
		LiveEntityIds: make([]uint32, prng.IntN(32)),
		Columns:       make([]*cardinalv1.Column, prng.IntN(16)),
	}
	for i := range worldState.GetLiveEntityIds() {
		worldState.LiveEntityIds[i] = prng.Uint32()
	}
	for i := range worldState.GetColumns() {
		entityIDs := make([]uint32, prng.IntN(32))
		payloads := make([][]byte, len(entityIDs))
		for j := range entityIDs {
			entityIDs[j] = prng.Uint32()
			payloads[j] = randomBytes(prng.IntN(256))
		}
		worldState.Columns[i] = &cardinalv1.Column{
			Name:      fmt.Sprintf("component-%d", prng.Uint64()),
			EntityIds: entityIDs,
			Payloads:  payloads,
		}
	}
