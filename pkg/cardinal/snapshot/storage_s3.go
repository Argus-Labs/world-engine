...
//
//	CARDINAL_S3_ENDPOINT=<url>         # Custom endpoint for S3-compatible services (e.g. http://localhost:9000 for MinIO)
...
func NewS3Storage(opts S3StorageOptions) (*S3Storage, error) {
	...
	if opts.Bucket == "" {
		return nil, eris.New("CARDINAL_S3_BUCKET environment variable is required")
	}

	// Build the S3 key using the same org/project/serviceId pattern as JetStream bucket naming.
	...
	// Build S3 client with optional custom endpoint for S3-compatible services.
