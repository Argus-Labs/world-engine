...
// Bench archetypes: structs of component types, resolved at first use by state.Contains/Exact/Create.
type arch1 struct{ Position Position3D }
...
func BenchmarkCardinal_Entity_Create(b *testing.B) {
	b.Run("1 component with archetype creation", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			w := newBenchWorld()
			state := newBenchState(w)

			b.StartTimer()
			entity := state.Create[arch1]()
...
func BenchmarkCardinal_Iteration_Pure(b *testing.B) {
	b.Run("Exact/1 component", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			b.StartTimer()
			for result := range state.Exact[arch1]().Iter() {
				_ = result
			}
			b.StopTimer()
		}
	...
	b.Run("Exact/5 components", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			b.StartTimer()
			for result := range state.Exact[arch5]().Iter() {
				_ = result
			}
			b.StopTimer()
		}
	...
	b.Run("Exact/10 components", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			b.StartTimer()
			for result := range state.Exact[arch10]().Iter() {
				_ = result
			}
			b.StopTimer()
		}
	...
	b.Run("Contains/1 from 1 component entities", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			b.StartTimer()
			for result := range state.Contains[arch1]().Iter() {
				_ = result
			}
			b.StopTimer()
		}
	...
	b.Run("Contains/3 from 5 component entities", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			// A real world resolves the archetype once, before its first tick; the timed
			// region measures the steady-state query build plus iteration.
			state.Contains[arch3]()

			b.StartTimer()
			for result := range state.Contains[arch3]().Iter() {
				_ = result
			}
			b.StopTimer()
		}
	...
	b.Run("Contains/6 from 10 component entities", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			// A real world resolves the archetype once, before its first tick; the timed
			// region measures the steady-state query build plus iteration.
			state.Contains[arch6]()

			b.StartTimer()
			for result := range state.Contains[arch6]().Iter() {
				_ = result
			}
			b.StopTimer()
		}
...
type getSetSystemState1 struct{ BaseSystemState }

type getSetSystemState5 struct{ BaseSystemState }

type getSetSystemState10 struct{ BaseSystemState }
...
func BenchmarkCardinal_Iteration_GetSet(b *testing.B) {
	b.Run("1 component 100 entities", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			w.RegisterSystem(func(state *getSetSystemState1) {
				b.StartTimer()
				for entity := range state.Contains[arch1]().Iter() {
	...
	b.Run("5 components 100 entities", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			w.RegisterSystem(func(state *getSetSystemState5) {
				b.StartTimer()
				for entity := range state.Contains[arch5]().Iter() {
					pos := entity.Get[Position3D]()
	...
	b.Run("10 components 100 entities", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			w.RegisterSystem(func(state *getSetSystemState10) {
				b.StartTimer()
				for entity := range state.Contains[arch10]().Iter() {
					pos := entity.Get[Position3D]()
...
// newBenchState binds a system state to w without registering a system.
func newBenchState(w *World) *BaseSystemState {
	return &BaseSystemState{world: w}
}
