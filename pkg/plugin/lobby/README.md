...
```sh
world sdk generate ./pkg/plugin/lobby \
  --go-out ./pkg/plugin/lobby/gen
```
...
> **`--proto-out ./pkg/plugin/lobby/proto` is optional.** The generator compiles the schema in a temp
...
func AssignerSystem(state *AssignerState) {
    ...
    for refs := range state.Contains[lobbyRow]().Iter() {
