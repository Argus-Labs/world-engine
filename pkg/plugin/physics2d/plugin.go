...
//
// The plugin's pipeline (reconcile + writeback in Update) owns the following body state.
// Calling these setters directly is ineffective — the values will be overwritten by the
...
// ECS components are unchanged. The next PhysicsPipelineSystem (Update) runs
...
func (p *Plugin) Register(world *cardinal.World) {
	...
	cardinal.RegisterSystem(world, physicssystem.PhysicsPipelineSystem, cardinal.WithHook(cardinal.Update))
}
