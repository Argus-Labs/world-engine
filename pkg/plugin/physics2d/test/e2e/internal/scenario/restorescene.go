...
// materials avoid the defaults; and every shape type is
...
func buildRestoreScene(c *harness.Ctx) {
	...
	c.Spawn("floor", 0, groundY,
		body(c, physics.BodyTypeStatic, withFriction(box(20, 1), 0.55)))
	for i := range 3 {
		c.Spawn(stackLabel(i), 0, 0.5+float64(i), body(c, physics.BodyTypeDynamic,
	...
	noSleep := hover(body(c, physics.BodyTypeDynamic, circle(0.5)))
	noSleep.SleepingAllowed = false
	c.Spawn("flag-never-sleeps", -22, 20, noSleep)

	bullet := hover(body(c, physics.BodyTypeDynamic, circle(0.25)))
	bullet.Bullet = true
	c.Spawn("flag-bullet", -18, 20, bullet)

	locked := hover(body(c, physics.BodyTypeDynamic, box(0.5, 0.5)))
	locked.FixedRotation = true
	c.SpawnSpinning("flag-fixed-rotation", -14, 20, 3, locked)

	// --- Gravity scales, on their own floor.
	c.Spawn("gravity-floor", -8, 30, body(c, physics.BodyTypeStatic, box(10, 1)))

	heavyG := body(c, physics.BodyTypeDynamic, circle(0.5))
	heavyG.GravityScale = 2.5
	c.Spawn("flag-gravity-2.5", -10, 40, heavyG)

	// Rises for the whole run with nothing above it.
	upwards := body(c, physics.BodyTypeDynamic, circle(0.5))
	...
	damped := hover(body(c, physics.BodyTypeDynamic, circle(0.5)))
	...
	// poses drift apart.
	c.Spawn("kind-floor", 16, 50, body(c, physics.BodyTypeStatic, box(14, 1)))
	c.Spawn("kind-static", 10, 60, body(c, physics.BodyTypeStatic, box(1, 1)))
	c.Spawn("kind-dynamic", 14, 60, body(c, physics.BodyTypeDynamic, box(0.5, 0.5)))
	...
	c.SpawnMoving("kind-manual", 24, 65, 7, -3, body(c, physics.BodyTypeManual, box(0.5, 0.5)))
	...
	c.Spawn("shape-floor", -24, 70, body(c, physics.BodyTypeStatic, box(8, 1)))
	c.Spawn("shape-circle", -30, 80, body(c, physics.BodyTypeDynamic, circle(0.65)))
	c.Spawn("shape-box", -26, 80, body(c, physics.BodyTypeDynamic, box(0.4, 0.7)))
	c.Spawn("shape-polygon", -22, 80, body(c, physics.BodyTypeDynamic,
		polygon(vec(-0.6, -0.4), vec(0.6, -0.4), vec(0.3, 0.5), vec(-0.3, 0.5))))
	c.Spawn("shape-capsule", -18, 80, body(c, physics.BodyTypeDynamic,
		capsule(vec(-0.45, 0), vec(0.45, 0), 0.28)))
	c.Spawn("shape-chain", -10, 80, body(c, physics.BodyTypeStatic,
		chain(vec(3, 0), vec(1, 0), vec(-1, 0), vec(-3, 0))))
	c.Spawn("shape-chain-loop", 0, 80, body(c, physics.BodyTypeStatic,
	...
	c.Spawn("filter-high-bits", -30, 120, body(c, physics.BodyTypeStatic,
		withFilter(box(1, 1), 1<<40, (1<<40)|(1<<55), 0)))
	c.Spawn("filter-negative-group", -24, 120, body(c, physics.BodyTypeStatic,
		withFilter(box(1, 1), 0x20, 0xFF, -9)))
	c.Spawn("filter-positive-group", -18, 120, body(c, physics.BodyTypeStatic,
	...
	c.Spawn("sensor-gate", 0, 140, body(c, physics.BodyTypeStatic, asSensor(box(3, 3))))
	visitor := hover(body(c, physics.BodyTypeDynamic, circle(0.5)))
	...
	c.Spawn("awake-drift-floor", 40, -80, body(c, physics.BodyTypeStatic, box(6, 1)))
	dormant := body(c, physics.BodyTypeDynamic, box(0.6, 0.6))
	dormant.Awake = false
	c.Spawn("awake-drift-sleeper", 40, 200, dormant)
	c.Spawn("awake-drift-waker", 40, 212,
		body(c, physics.BodyTypeDynamic, withRestitution(box(0.6, 0.6), 0.95)))
}
