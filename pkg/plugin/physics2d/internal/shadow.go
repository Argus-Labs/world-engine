...
// entity. Every field is a plain value (the slot list is immutable), so the copy is the
// snapshot. Shape contents are not shadowed: shapes live on their own entities and the
// runtime's ShapeMirror tracks their changes (see SyncShapes).
...
// NewShadowState returns a shadow snapshot of the three components.
func NewShadowState(
...
func (s ShadowState) ShapesDiffer(p component.PhysicsBody2D) bool {
	return !immutable.Equal(s.PhysicsBody.Shapes, p.Shapes)
}
