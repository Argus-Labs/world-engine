...
e2e-nakama:
	...
	@(set -o pipefail; \
	    ...
	    sleep 2 && \
		docker compose up --build chain game nakama test_nakama --abort-on-container-exit --exit-code-from test_nakama 2>&1) | grep --color=always "test_nakama  "; \
		docker compose rm --force --stop)
...
define check_url
	...
	while [ $$attempts -lt $(3) ]; do \
		...
		while [ $$(( $$(date +%s) - start )) -lt $$timeout ]; do \
			if curl -s -o /dev/null -w "%{http_code}" "$(1)" -m 1 2>/dev/null | grep -q "$(2)"; then \
		...
		if [ $$attempts -lt $(3) ]; then \
			echo "Restarting $(4) service for attempt $$attempts"; \
...
swagger-check:
	...
	cd cardinal && \
		docker run --rm -v "$(PWD)/cardinal:/local-repo" ghcr.io/argus-labs/devops-infra-swagger-diff:2.0.0 \
		/local-repo/server/docs/swagger.json /local-repo/.tmp/swagger/swagger.json && \
		echo "swagger-diff: no changes detected"

	@echo "--> Cleanup"
	rm -rf "cardinal/.tmp/swagger"
