...
type Snapshot struct {
	...
	DiskState []byte `protobuf:"bytes,5,opt,name=disk_state,json=diskState,proto3" json:"disk_state,omitempty"`
	// SHA-256 checksum of disk_state for integrity verification on restore.
	DiskStateChecksum []byte `protobuf:"bytes,6,opt,name=disk_state_checksum,json=diskStateChecksum,proto3" json:"disk_state_checksum,omitempty"`
...
func (x *Snapshot) GetDiskState() []byte {
	if x != nil {
		return x.DiskState
	}
	return nil
...
func (x *Snapshot) GetDiskStateChecksum() []byte {
	if x != nil {
		return x.DiskStateChecksum
	}
	return nil
}
