...
func TestReadTypeNotStructs(t *testing.T) {
	...
	defer func() {
		...
		NewReadType[FooRequest, FooReply]("foo", func(world public.IWorld, req FooRequest) (FooReply, error) {
	...
	NewReadType[string, string]("foo", func(world public.IWorld, req string) (string, error) {
		return "blah", nil
	})
}
...
func TestReadEVM(t *testing.T) {
	...
	fooRead := NewReadType[FooRequest, FooReply]("foo", func(world public.IWorld, req FooRequest) (FooReply, error) {
		return expectedReply, nil
	}, WithReadEVMSupport[FooRequest, FooReply])

	w := ecs.NewTestWorld(t)
	err := w.RegisterReads(fooRead)
	err = w.RegisterTransactions(transaction.NewTransactionType[struct{}, struct{}]("blah"))
