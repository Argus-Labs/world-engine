...
func TestReadTypeNotStructs(t *testing.T) {
	...
	defer func() {
		...
		transaction.NewTransactionType[*ModifyScoreTx, *EmptyTxResult]("modify_score2")

	}()

	transaction.NewTransactionType[string, string]("modify_score1")
...
func TestCanQueueTransactions(t *testing.T) {
	...
	score := component.NewComponentType[*ScoreComponent]("score")
	assert.NilError(t, world.RegisterComponents(score))
	modifyScoreTx := transaction.NewTransactionType[*ModifyScoreTx, *EmptyTxResult]("modify_score")
	...
	world.AddSystem(func(w *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		for _, txData := range modifyScore {
			ms := txData.Value
			err := score.Update(w.Logger, w.NameToComponent(), w.StoreManager(), ms.PlayerID, func(s *ScoreComponent) *ScoreComponent {
				s.Score += ms.Amount
				return s
			})
			if err != nil {
	...
	modifyScoreTx.AddToQueue(world.GetTxQueue(), &ModifyScoreTx{id, 100})

	assert.NilError(t, score.Set(world.Logger, world.NameToComponent(), world.StoreManager(), id, &ScoreComponent{}))

	// Verify the score is 0
	s, err := score.Get(world.StoreManager(), id)
	assert.NilError(t, err)
...
func TestSystemsAreExecutedDuringGameTick(t *testing.T) {
	...
	count := component.NewComponentType[CounterComponent]("count")
	...
	world.AddSystem(func(w *ecs.World, _ *transaction.TxQueue, _ *log.Logger) error {
		return count.Update(w.Logger, w.NameToComponent(), w.StoreManager(), id, func(c CounterComponent) CounterComponent {
			c.Count++
			return c
		})
	})
...
func TestTransactionAreAppliedToSomeEntities(t *testing.T) {
	world := ecs.NewTestWorld(t)
	alphaScore := component.NewComponentType[ScoreComponent]("alphaScore")
	...
	world.AddSystem(func(w *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		for _, msData := range modifyScores {
			ms := msData.Value
			err := alphaScore.Update(w.Logger, w.NameToComponent(), w.StoreManager(), ms.PlayerID, func(s ScoreComponent) ScoreComponent {
				s.Score += ms.Amount
				return s
			})
	...
	modifyScoreTx.AddToQueue(world.GetTxQueue(), &ModifyScoreTx{
		PlayerID: ids[5],
		Amount:   105,
	})
	modifyScoreTx.AddToQueue(world.GetTxQueue(), &ModifyScoreTx{
	...
	for i, id := range ids {
		...
		s, err := alphaScore.Get(world.StoreManager(), id)
		assert.NilError(t, err)
...
func TestAddToQueueDuringTickDoesNotTimeout(t *testing.T) {
	...
	modScore := transaction.NewTransactionType[*ModifyScoreTx, *EmptyTxResult]("modify_Score")
	...
	modScore.AddToQueue(world.GetTxQueue(), &ModifyScoreTx{})
...
func TestTransactionsAreExecutedAtNextTick(t *testing.T) {
	world := ecs.NewTestWorld(t)
	modScoreTx := transaction.NewTransactionType[*ModifyScoreTx, *EmptyTxResult]("modify_score")
	...
	modScoreTx.AddToQueue(world.GetTxQueue(), &ModifyScoreTx{})
...
func TestIdenticallyTypedTransactionCanBeDistinguished(t *testing.T) {
	...
	alpha := transaction.NewTransactionType[NewOwner, EmptyTxResult]("alpha_tx")
	beta := transaction.NewTransactionType[NewOwner, EmptyTxResult]("beta_tx")
	assert.NilError(t, world.RegisterTransactions(alpha, beta))

	alpha.AddToQueue(world.GetTxQueue(), NewOwner{"alpha"})
	beta.AddToQueue(world.GetTxQueue(), NewOwner{"beta"})
...
func TestCannotRegisterDuplicateTransaction(t *testing.T) {
	tx := transaction.NewTransactionType[ModifyScoreTx, EmptyTxResult]("modify_score")
	world := ecs.NewTestWorld(t)
...
func TestCanEncodeDecodeEVMTransactions(t *testing.T) {
	...
	itx := transaction.NewTransactionType[FooTx, EmptyTxResult]("FooTx", transaction.WithTxEVMSupport[FooTx, EmptyTxResult])
...
func TestCannotDecodeEVMBeforeSetEVM(t *testing.T) {
	type foo struct{}
	tx := transaction.NewTransactionType[foo, EmptyTxResult]("foo")
	_, err := tx.DecodeEVMBytes([]byte{})
	assert.ErrorIs(t, err, transaction.ErrEVMTypeNotSet)
}
...
func TestCanGetTransactionErrorsAndResults(t *testing.T) {
	...
	moveTx := transaction.NewTransactionType[MoveTx, MoveTxResult]("move")
	...
	world.AddSystem(func(world *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		moveTx.AddError(world.GetReceiptHistory(), tx.TxHash, wantFirstError)
		moveTx.AddError(world.GetReceiptHistory(), tx.TxHash, wantSecondError)
		...
		moveTx.SetResult(world.GetReceiptHistory(), tx.TxHash, MoveTxResult{42, 42})
		return nil
	})
	assert.NilError(t, world.LoadGameState())
	_ = moveTx.AddToQueue(world.GetTxQueue(), MoveTx{99, 100})
...
func TestSystemCanFindErrorsFromEarlierSystem(t *testing.T) {
	...
	world := ecs.NewTestWorld(t)
	numTx := transaction.NewTransactionType[TxIn, TxOut]("number")
	...
	world.AddSystem(func(world *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		hash := txs[0].TxHash
		_, _, ok := numTx.GetReceipt(world.GetReceiptHistory(), hash)
		assert.Check(t, !ok)
		numTx.AddError(world.GetReceiptHistory(), hash, wantErr)
		return nil
	})
	...
	world.AddSystem(func(world *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		assert.Equal(t, 1, len(txs))
		hash := txs[0].TxHash
		_, errs, ok := numTx.GetReceipt(world.GetReceiptHistory(), hash)
		assert.Check(t, ok)
	...
	_ = numTx.AddToQueue(world.GetTxQueue(), TxIn{100})
...
func TestSystemCanClobberTransactionResult(t *testing.T) {
	...
	world.AddSystem(func(world *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		_, _, ok := numTx.GetReceipt(world.GetReceiptHistory(), hash)
		assert.Check(t, !ok)
		numTx.SetResult(world.GetReceiptHistory(), hash, firstResult)
		return nil
	})
	...
	world.AddSystem(func(world *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		...
		assert.Equal(t, 1, len(txs))
		hash := txs[0].TxHash
		out, errs, ok := numTx.GetReceipt(world.GetReceiptHistory(), hash)
		assert.Check(t, ok)
		assert.Equal(t, 0, len(errs))
		assert.Equal(t, TxOut{1234}, out)
		numTx.SetResult(world.GetReceiptHistory(), hash, secondResult)
		return nil
	})
	assert.NilError(t, world.LoadGameState())
