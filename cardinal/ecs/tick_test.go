...
func TestCanIdentifyAndFixSystemError(t *testing.T) {
	...
	oneWorld.AddSystem(func(world *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
		p, err := onePower.Get(world.StoreManager(), id)
		if err != nil {
			return err
		}
		p.Power++
		...
		return onePower.Set(world.Logger, world.NameToComponent(), world.StoreManager(), id, p)
	})
	assert.NilError(t, oneWorld.LoadGameState())
	...
	assert.NilError(t, twoWorld.LoadGameState())
	p, err := onePower.Get(twoWorld.StoreManager(), id)
	assert.NilError(t, err)
...
func TestCanModifyArchetypeAndGetEntity(t *testing.T) {
	...
	assert.NilError(t, alpha.Set(world.Logger, world.NameToComponent(), world.StoreManager(), wantID, wantScalar))

	verifyCanFindEntity := func() {
		// Make sure we can find the entity
		gotID, err := alpha.First(world.Namespace(), world.Store())
		assert.NilError(t, err)
		assert.Equal(t, wantID, gotID)

		// Make sure the associated component is correct
		gotScalar, err := alpha.Get(world.StoreManager(), wantID)
		assert.NilError(t, err)
	...
	assert.NilError(t, beta.AddTo(world.StoreManager(), wantID))
	verifyCanFindEntity()

	// Remove the beta component
	assert.NilError(t, beta.RemoveFrom(world.StoreManager(), wantID))
	verifyCanFindEntity()
}
...
func TestCanRecoverStateAfterFailedArchetypeChange(t *testing.T) {
	...
	for _, firstWorldIteration := range []bool{true, false} {
		...
		world.AddSystem(func(w *ecs.World, _ *transaction.TxQueue, _ *log.Logger) error {
			...
			id, err := static.First(w.Namespace(), w.Store())
			assert.NilError(t, err)

			s, err := static.Get(w.StoreManager(), id)
			assert.NilError(t, err)
			s.Val++
			assert.NilError(t, static.Set(w.Logger, w.NameToComponent(), w.StoreManager(), id, s))
			if s.Val%2 == 1 {
				assert.NilError(t, toggle.AddTo(w.StoreManager(), id))
			} else {
				assert.NilError(t, toggle.RemoveFrom(w.StoreManager(), id))
			}
		...
		id, err := static.First(world.Namespace(), world.Store())
		assert.NilError(t, err)
		...
		if firstWorldIteration {
			...
			_, err := toggle.Get(world.StoreManager(), id)
			...
			s, err := static.Get(world.StoreManager(), id)
			assert.NilError(t, err)
...
func TestCanRecoverTransactionsFromFailedSystemRun(t *testing.T) {
	...
	for _, isBuggyIteration := range []bool{true, false} {
		...
		world.AddSystem(func(w *ecs.World, queue *transaction.TxQueue, _ *log.Logger) error {
			id := powerComp.MustFirst(w.Namespace(), w.Store())
			entityPower, err := powerComp.Get(w.StoreManager(), id)
			assert.NilError(t, err)

			changes := powerTx.In(queue)
			assert.Equal(t, 1, len(changes))
			entityPower.Val += changes[0].Value.Val
			assert.NilError(t, powerComp.Set(w.Logger, w.NameToComponent(), w.StoreManager(), id, entityPower))
		...
		fetchPower := func() float64 {
			id, err := powerComp.First(world.Namespace(), world.Store())
			assert.NilError(t, err)
			power, err := powerComp.Get(world.StoreManager(), id)
			assert.NilError(t, err)
			return power.Val
		}
		...
		if isBuggyIteration {
			...
			powerTx.AddToQueue(world.GetTxQueue(), FloatValue{1000})
			assert.NilError(t, world.Tick(context.Background()))
			powerTx.AddToQueue(world.GetTxQueue(), FloatValue{1000})
			assert.NilError(t, world.Tick(context.Background()))
			powerTx.AddToQueue(world.GetTxQueue(), FloatValue{1000})
			...
			powerTx.AddToQueue(world.GetTxQueue(), FloatValue{666})
