...
func TestCanFilterByArchetype(t *testing.T) {
	...
	component.NewQuery(filter.Exact(alpha, beta)).Each(world, func(id public.EntityID) bool {
		count++
...
func TestExactVsContains(t *testing.T) {
	...
	component.NewQuery(filter.Contains(alpha)).Each(world, func(id public.EntityID) bool {
		count++
		return true
	})
	...
	assert.NilError(t, err)
	component.NewQuery(sameQuery).Each(world, func(id public.EntityID) bool {
		count2++
		return true
	})
	...
	component.NewQuery(filter.Contains(beta)).Each(world, func(id public.EntityID) bool {
		count++
		return true
	})
	...
	component.NewQuery(filter.Exact(alpha)).Each(world, func(id public.EntityID) bool {
		count++
		return true
	})
	...
	component.NewQuery(filter.Exact(beta, alpha)).Each(world, func(id public.EntityID) bool {
		count++
		return true
	})
	assert.Equal(t, count, bothCount)
...
func TestCanGetArchetypeFromEntity(t *testing.T) {
	...
	count := 0
	component.NewQuery(filter.Exact(comps...)).Each(world, func(id public.EntityID) bool {
		count++
		return true
	})
