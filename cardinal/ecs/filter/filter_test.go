...
func TestCanFilterByArchetype(t *testing.T) {
	...
	ecs.NewQuery(filter.Exact(alpha, beta)).Each(world, func(id interfaces.EntityID) bool {
		count++
...
func TestExactVsContains(t *testing.T) {
	...
	ecs.NewQuery(filter.Contains(alpha)).Each(world, func(id interfaces.EntityID) bool {
		count++
		return true
	})
	...
	assert.NilError(t, err)
	ecs.NewQuery(sameQuery).Each(world, func(id interfaces.EntityID) bool {
		count2++
		return true
	})
	...
	ecs.NewQuery(filter.Contains(beta)).Each(world, func(id interfaces.EntityID) bool {
		count++
		return true
	})
	...
	ecs.NewQuery(filter.Exact(alpha)).Each(world, func(id interfaces.EntityID) bool {
		count++
		return true
	})
	...
	ecs.NewQuery(filter.Exact(beta, alpha)).Each(world, func(id interfaces.EntityID) bool {
		count++
		return true
	})
	assert.Equal(t, count, bothCount)
...
func TestCanGetArchetypeFromEntity(t *testing.T) {
	...
	count := 0
	ecs.NewQuery(filter.Exact(comps...)).Each(world, func(id interfaces.EntityID) bool {
		count++
		return true
	})
