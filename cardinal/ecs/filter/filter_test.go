...
func TestCanFilterByArchetype(t *testing.T) {
	...
	subsetCount := 50
	_, err := cardinal.CreateMany(world, subsetCount, Alpha{}, Beta{})
	assert.NilError(t, err)
	// Make some entities that have all 3 component.
	_, err = cardinal.CreateMany(world, 20, Alpha{}, Beta{}, Gamma{})
	assert.NilError(t, err)
	...
	q.Each(world, func(id entity.ID) bool {
		...
		_, err := cardinal.GetComponent[gammaComponent](world, id)
...
func TestExactVsContains(t *testing.T) {
	...
	alphaCount := 75
	_, err := cardinal.CreateMany(world, alphaCount, Alpha{})
	assert.NilError(t, err)
	bothCount := 100
	_, err = cardinal.CreateMany(world, bothCount, Alpha{}, Beta{})
	assert.NilError(t, err)
	count := 0
...
func TestCanGetArchetypeFromEntity(t *testing.T) {
	...
	wantCount := 50
	ids, err := cardinal.CreateMany(world, wantCount, Alpha{}, Beta{})
	assert.NilError(t, err)
	// Make some extra entities that will be ignored. Our query later
	// should NOT contain these entities
	_, err = cardinal.CreateMany(world, 20, Alpha{})
	assert.NilError(t, err)
	id := ids[0]
...
func BenchmarkEntityCreation(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		_, err := cardinal.CreateMany(world, 100000, Alpha{})
...
func helperArchetypeFilter(b *testing.B, relevantCount, ignoreCount int) {
	...
	assert.NilError(b, world.LoadGameState())
	_, err := cardinal.CreateMany(world, relevantCount, Alpha{}, Beta{})
	assert.NilError(b, err)
	_, err = cardinal.CreateMany(world, ignoreCount, Alpha{})
	assert.NilError(b, err)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		count := 0
