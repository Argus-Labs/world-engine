...
func TestErrorWhenSavedArchetypesDoNotMatchComponentTypes(t *testing.T) {
	...
	_, err := ecs.Create(cardinal.NewWorldContext(oneWorld), OneAlphaNum{})
...
func TestArchetypeIDIsConsistentAfterSaveAndLoad(t *testing.T) {
	...
	_, err := ecs.Create(cardinal.NewWorldContext(oneWorld), NumberComponent{})
	assert.NilError(t, err)
...
func TestCanReloadState(t *testing.T) {
	...
	_, err = ecs.CreateMany(cardinal.NewWorldContext(alphaWorld), 10, oneAlphaNumComp{})
	assert.NilError(t, err)
