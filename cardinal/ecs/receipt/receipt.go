...
func NewHistory(currentTick uint64, ticksToStore int) *History {
	...
	h.history = make([]map[message.Hash]Receipt, 0, ticksToStore)
	for i := 0; i < ticksToStore; i++ {
		h.history = append(h.history, map[message.Hash]Receipt{})
	}
...
func (h *History) NextTick() {
	...
	h.history[mod] = map[message.Hash]Receipt{}
}
...
func (h *History) AddError(hash message.Hash, err error) {
...
// results.
func (h *History) SetResult(hash message.Hash, result any) {
	tick := int(h.currTick.Load() % h.ticksToStore)
	rec := h.history[tick][hash]
	rec.TxHash = hash
	rec.Result = result
...
func (h *History) GetReceipt(hash message.Hash) (Receipt, bool) {
	tick := int(h.currTick.Load() % h.ticksToStore)
