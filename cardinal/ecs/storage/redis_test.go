...
import (
	...
	transaction2 "pkg.world.dev/world-engine/cardinal/ecs/transaction"
...
func TestRedis_Location(t *testing.T) {
	...
	assert.Equal(t, loc.GetCompIndex(), gotLoc.GetCompIndex())

	aid, _ := store.EntityLocStore.ArchetypeID(eid)
	assert.Equal(t, loc.GetArchID(), aid)
	...
	assert.Equal(t, newLoc.GetArchID(), archID2)
	assert.Equal(t, newLoc.GetCompIndex(), compIdx2)
...
func TestCanSaveAndRecoverSignatures(t *testing.T) {
	...
	tx := transaction2.NewTransactionType[TxIn, TxOut]("tx_a")
	tx.SetID(55)
