...
type Query struct {
	archMatches map[public.WorldNamespace]*cache
...
func NewQuery(filter public.IComponentFilter) *Query {
	return &Query{
		archMatches: make(map[public.WorldNamespace]*cache),
		filter:      filter,
	}
}
...
func (q *Query) Each(w public.IWorld, callback public.QueryCallBackFn) {
...
func (q *Query) evaluateQuery(world public.IWorld) []public.ArchetypeID {
	w := public.WorldNamespace(world.Namespace())
