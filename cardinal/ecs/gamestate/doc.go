...
EntityComponentBuffer.AtomicFn takes in a function that returns an error. The passed in function will be executed, and
any state made during that function call will be stored as pending state changes. During this time, reads using the
EntityComponentBuffer will report the pending values. Conversely, reading data directly from Redis the original value
...
Alternatively, EntityComponentBuffer can be used outside an AtomicFn context. State changes are stored as pending
...
Pending changes can be discarded with EntityComponentBuffer.DiscardPending. A subsequent read will return identical
data to the data stored in Redis.

Pending changes can be committed to redis with EntityComponentBuffer.FinalizeTick. All pending changes will
