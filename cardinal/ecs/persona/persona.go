...
func buildPersonaTagMapping(world public.IWorld) (map[string]personaTagComponentData, error) {
	...
	var errs []error
	component.NewQuery(filter.Exact(SignerComp)).Each(world, func(id public.EntityID) bool {
...
func GetSignerForPersonaTag(w public.IWorld, personaTag string, tick uint64) (addr string, err error) {
	if tick >= w.CurrentTick() {
		return "", ErrorCreatePersonaTxsNotProcessed
	}
	var errs []error
	component.NewQuery(filter.Exact(SignerComp)).Each(w, func(id public.EntityID) bool {
