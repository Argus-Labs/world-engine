...
func TestCanCreateAndVerifyPersonaSigner(t *testing.T) {
	...
	signedPayload, err := sign.NewSystemSignedPayload(privateKey, world.NamespaceAsString(), 100, createPersonaTx)
	assert.NilError(t, err)
...
func TestSigVerificationChecksNamespace(t *testing.T) {
	...
	sigPayload, err = sign.NewSystemSignedPayload(privateKey, world.NamespaceAsString(), 100, createPersonaTx)
	assert.NilError(t, err)
	bz, err = sigPayload.Marshal()
	assert.NilError(t, err)
	resp, err = http.Post(txh.makeHttpURL(url), "application/json", bytes.NewReader(bz))
...
func TestSigVerificationChecksNonce(t *testing.T) {
	...
	personaTag := "some_dude"
	signerAddr := crypto.PubkeyToAddress(privateKey.PublicKey).Hex()
	namespace := world.NamespaceAsString()
...
func TestTransactionsSubmittedToChain(t *testing.T) {
	...
	personaTag := "clifford_the_big_red_dog"
	signerAddr := crypto.PubkeyToAddress(privateKey.PublicKey).Hex()
	sigPayload, err := sign.NewSystemSignedPayload(privateKey, world.NamespaceAsString(), 1, ecs.CreatePersonaTransaction{
		PersonaTag:    personaTag,
		SignerAddress: signerAddr,
	})
	assert.NilError(t, err)
	...
	sigPayload, err = sign.NewSignedPayload(privateKey, personaTag, world.NamespaceAsString(), 2, MoveTx{Direction: "up"})
	assert.NilError(t, err)
	bz, err = sigPayload.Marshal()
	assert.NilError(t, err)
