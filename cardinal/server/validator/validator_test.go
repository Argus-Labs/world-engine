...
func (s *ValidatorTestSuite) TestCanSendTxWithoutSigVerification() {
	...
	//msg := server.MoveMsgInput{Direction: "up"}
	...
	//	Body:       msgBz,
	//}
	//moveMessage, ok := s.world.GetMessageByFullName("game." + server.moveMsgName)
	//s.Require().True(ok)
	...
	//res = s.fixture.Post("query/game/location", server.QueryLocationRequest{Persona: persona})
	//var loc server.LocationComponent
	//err = json.Unmarshal([]byte(s.readBody(res.Body)), &loc)
	//s.Require().NoError(err)
	//s.Require().Equal(server.LocationComponent{0, 1}, loc)
}
...
func (s *ValidatorTestSuite) TestMissingSignerAddressIsOKWhenSigVerificationIsDisabled() {
	...
	//// encounter no errors
	//payload := server.MoveMsgInput{Direction: "up"}
	//
...
func (s *ValidatorTestSuite) TestRejectDuplicateTransactionHash() {
	...
	//tx, err := sign.NewTransaction(s.privateKey, personaTag, s.world.Namespace(), server.MoveMsgInput{Direction: "up"})
	//s.Require().NoError(err)
	//
