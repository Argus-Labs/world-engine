...
func NewIterator(r Router) *TxIterator {
	rtr := r.(*router)
	return &TxIterator{
		provider:  rtr.provider,
		namespace: rtr.namespace,
		querier:   rtr.ShardQuerier,
	}
}
	...
	for {
		...
		for _, epoch := range res.Epochs {
			...
			for _, tx := range epoch.Txs {
				msgType, exists := t.provider.GetMessageByID(message.TypeID(tx.TxId))
				if !exists {
