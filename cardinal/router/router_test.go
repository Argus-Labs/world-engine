...
//
//	provider.EXPECT().GetMessageByFullName(name).Return(msg, true).Times(1)
//	provider.EXPECT().GetSignerComponentForPersona(personaTag).Return(nil, fmt.Errorf("not found")).Times(1)
//
//	res, err := router.server.SendMessage(
//		context.Background(),
//		&routerv1.SendMessageRequest{MessageId: name, PersonaTag: personaTag, Sender: sender},
//	)
//	assert.NilError(t, err)
...
//	provider.EXPECT().
//		GetSignerComponentForPersona(personaTag).
//		Return(&persona.Persona{AuthorizedAddresses: []string{sender}}, nil).
//		Times(1)
//	provider.EXPECT().AddEVMTransaction(msg.id, msgValue, &sign.Transaction{PersonaTag: personaTag}, evmTxHash).Times(1)
...
//	provider.EXPECT().
//		GetSignerComponentForPersona(personaTag).
//		Return(&persona.Persona{AuthorizedAddresses: []string{"bogus"}}, nil).
//		Times(1)
//
//	res, err := router.server.SendMessage(context.Background(), req)
//	assert.NilError(t, err)
