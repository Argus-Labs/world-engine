...
type World struct {
	...
	txsInPool                int
	mux                      *sync.Mutex
	registeredMessages       map[string]message.MessageType
	registeredMessagesByType map[reflect.Type]message.MessageType
...
func New(rs *redis.Storage, opts ...Option) (*World, error) {
	...
	w := &World{
		...
		// Tx
		txMap:                    make(types.TxMap),
		txsInPool:                0,
		mux:                      new(sync.Mutex),
		registeredMessages:       make(map[string]message.MessageType),
		registeredMessagesByType: make(map[reflect.Type]message.MessageType),
