	...
	for _, firstEngineIteration := range []bool{true, false} {
		...
		err = RegisterSystems(
			...
			func(wCtx engine.Context) error {
				...
				q := NewSearch(wCtx).Entity(filter.Contains(filter.Component[ScalarComponentStatic]()))
				id, err := q.First()
				assert.NilError(t, err)
...
func TestCanRecoverTransactionsFromFailedSystemRun(t *testing.T) {
	...
	for _, isBuggyIteration := range []bool{true, false} {
		...
		err = RegisterSystems(
			world,
			func(wCtx engine.Context) error {
				q := NewSearch(wCtx).Entity(filter.Contains(filter.Component[PowerComp]()))
				id := q.MustFirst()
...
func TestCanIdentifyAndFixSystemError(t *testing.T) {
	...
	err = RegisterSystems(
		world,
		func(wCtx engine.Context) error {
			searchObject := NewSearch(wCtx).Entity(filter.Exact(filter.Component[onePowerComponent]()))
			id := searchObject.MustFirst()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NilError(t, RegisterSystems(world, func(wCtx engine.Context) error {
				...
				id, err := NewSearch(wCtx).Entity(filter.Exact(filter.Component[Foo](), filter.Component[Bar]())).First()
				assert.Check(t, err == nil)
