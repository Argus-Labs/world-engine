...
func cardinalCreatePersona(ctx context.Context, nk runtime.NakamaModule, personaTag string) (
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, makeURL(createPersonaEndpoint), bytes.NewReader(buf))
	if err != nil {
...
func cardinalQueryPersonaSigner(ctx context.Context, personaTag string, tick uint64) (signerAddress string, err error) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, makeURL(readPersonaSignerEndpoint),
		bytes.NewReader(buf))
	if err != nil {
		return "", err
	}
