...
// enableAllowListForTest enables the allow list (beta keys) for the current test, then restores the allowlist
// flag back to it's original value.
func enableAllowListForTest(t *testing.T) {
	orig := allowlistEnabled
	t.Cleanup(func() {
		allowlistEnabled = orig
	})
